...
module "backup_kms" {
  ...
  warm_key_description    = var.warm_key_description
  archive_key_description = var.archive_key_description

  key_administrator_arns = local.key_admins
  key_user_arns          = local.key_users

  depends_on = [
    aws_iam_role.aws_backup_default_service_role,
    aws_iam_role_policy_attachment.backup_full_access,
    aws_iam_role_policy_attachment.backup_service_role_backup,
    aws_iam_role_policy_attachment.backup_service_role_restores
  ]
}
...
resource "aws_backup_vault_lock_configuration" "warm_use1" {
  ...
  depends_on = [aws_backup_vault.warm_use1]
}
...
resource "aws_backup_vault_lock_configuration" "archive_use2" {
  ...
  depends_on = [aws_backup_vault.archive_use2]
}
...
resource "aws_backup_plan" "ec2_two_tier_use1" {
  ...
  depends_on = [
    aws_backup_vault.warm_use1,
    aws_backup_vault.archive_use2,
    aws_backup_vault_lock_configuration.warm_use1,
    aws_backup_vault_lock_configuration.archive_use2
  ]
}
...
resource "aws_backup_selection" "daily_ec2_instances" {
  ...
  depends_on = [
    aws_backup_plan.ec2_two_tier_use1,
    aws_iam_role.aws_backup_default_service_role
  ]
}
...
resource "aws_sns_topic_subscription" "backup_failure_email" {
  ...
  depends_on = [aws_sns_topic.backup_failure_alerts]
}
...
resource "aws_cloudwatch_event_target" "backup_job_failure_to_sns" {
  ...
  depends_on = [
    aws_cloudwatch_event_rule.backup_job_failure,
    aws_sns_topic.backup_failure_alerts
  ]
}
...
resource "aws_cloudwatch_event_target" "copy_job_failure_to_sns" {
  ...
  depends_on = [
    aws_cloudwatch_event_rule.copy_job_failure,
    aws_sns_topic.backup_failure_alerts
  ]
}
...
resource "aws_cloudwatch_event_target" "restore_job_failure_to_sns" {
  ...
  depends_on = [
    aws_cloudwatch_event_rule.restore_job_failure,
    aws_sns_topic.backup_failure_alerts
  ]
}
